    ...
    //height and width
    const model_height = 400 - model_margin.top - model_margin.bottom;
    const model_width = 600 - model_margin.left - model_margin.right;

    //array used to populate dropdown menu, shared and used by other sections
    let sd_arr = [];
    ...
    d3.csv('../assets/raw_data/key_drivers.csv', function (error, data) {
        ...
        //populate sd_arr and the dropdown
        if (sd_arr.length == 0) {
        ...
        function setupSlider(y1, y2) {
            let sliderVals = [y1, y2];
            ...
            let handle = rangeSlider.selectAll('rect')
                ...
                .attr('class', 'handle')
                .attr('y', -10)
                .attr('x', d => { return xScale(sliderVals[d]); })
                .attr('rx', 3)
                .attr('height', 20)
                .attr('width', 16)
                .call(
        ...
        function updateGraph(yr1, yr2) {
            ...
            let districtData = data.filter(d => {
            ...
            districtData.forEach(d => {
                ...
                d.drivers = driverNames.map(name => {
                    return {
                        name: name,
                        value: +d[name]
                    };
            ...
            //set scales
            model_xScale.domain(districtData.map(d => { return d.SCHOOL_YEAR; }));
            bar_xScale1.domain(districtData.map(d => { return d.SCHOOL_YEAR; }));
            // set x2 scale within the bar group
            bar_xScale2.domain(driverNames).rangeRound([0, bar_xScale1.bandwidth()]);
            model_yScale.domain([0, d3.max(districtData, d => { return d.LAST_YEAR_ENROLMENT; })]);
            ...
            bar_yScale.domain([d3.min(districtData, d => {
                return d3.min(d.drivers, d => {
                    return d.value;
                });
            }), d3.max(districtData, d => {
                return d3.max(d.drivers, d => {
                    return d.value;
                });
            })]);
            ...
            yrGroup.selectAll('.bar')
                .data(d => { return d.drivers; })
                .enter().append('rect')
                .attr('class', 'bar')
                .attr('x', d => { return bar_xScale2(d.name); })
                .attr('width', d => { return bar_xScale2.bandwidth(); })
                .attr('y', d => { return bar_yScale(Math.max(0, d.value)); })
                .attr('height', d => { return Math.abs(bar_yScale(d.value) - bar_yScale(0)); })
                .style('fill', d => { return color_scale(d.name); })
                .on("mouseenter", function (d) {
                    /*Arrow functions are bound to their outer function, but if there is no wrapping function 
                    then the value will be undefined. In order to use the local this scope, don't use arrow 
                    function for mouse events here
                    */
            ...
            let dots = model_chartGroup.selectAll(".dot")
                ...
                .attr("cx", d => { return model_xScale(d.SCHOOL_YEAR); })
                .attr("cy", d => { return model_yScale(d.LAST_YEAR_ENROLMENT) })
                .attr("r", 5)
                .on("mouseenter", function (d) {
            ...
            $('#model_distDropdown').unbind().on('click', function (et) {
                ...
                d3.selectAll('#model_distDropdown .list div')
                    .on('click', function () {
                        ...
                        let districtData = data.filter(d => { return d.ABBREV == targetDistrict && (+d.SCHOOL_YEAR >= yr1 && +d.SCHOOL_YEAR <= yr2) });
                        ...
                        zero_line.y(d => { return bar_yScale(d['y']); });
