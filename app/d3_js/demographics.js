$(document).ready(function () {
    ...
    //height and width
    const demo_height = 400 - demo_margin.top - demo_margin.bottom;
    const demo_width = 600 - demo_margin.left - demo_margin.right;

    //parse string into year format from the csv file 
    ...
    d3.csv('../assets/raw_data/demographics.csv', function (error, data) {
        ...
        //array of selected district (objects)
        ...
        function demoUpdate(type, selectedDistricts) {
            ...
            for (let i = 0; i < selectedDistricts.length; i++) {
                ...
                demo_xScale.domain(districtData.map(d => {
                ...
                if ((selectedDistricts[i] == 'SD99-Province')) {
                    ...
                    if (radioValue != 'NET') {
                        demo_yScale.domain([Math.min(35000, d3.min(districtData, d => {
                            return d[type];
                        })),
                        Math.max(-35000, d3.max(districtData, d => {
                            return d[type];
                        }))
                        ]);
                    } else {
                        demo_yScale.domain([Math.min(0, d3.min(districtData, d => {
                            return d[type];
                        })),
                        Math.max(0, d3.max(districtData, d => {
                            return d[type];
                        }))
                        ]);
                    }
        ...
        $.each(dist_arr, function (index, dist) {
